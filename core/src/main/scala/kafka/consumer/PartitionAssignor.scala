...
class RoundRobinAssignor() extends PartitionAssignor with Logging {
  ...
  def assign(ctx: AssignmentContext) = {
    ...
    if (ctx.consumersForTopic.size > 0) {
      ...
      var threadAssignor = CoreUtils.circularIterator(allThreadIds)
      ...
      allTopicPartitions.foreach(topicPartition => {
        threadAssignor = threadAssignor.dropWhile(threadId => !ctx.consumersForTopic(topicPartition.topic).contains(threadId))
